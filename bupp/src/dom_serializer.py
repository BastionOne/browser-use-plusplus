...
class DOMTreeSerializer:
	...
	def serialize_tree(
		...
		if hasattr(node, 'excluded_by_parent') and node.excluded_by_parent:
			formatted_text = []
			for child in node.children:
				child_text = DOMTreeSerializer.serialize_tree(child, include_attributes, depth, node_metadata)
				if child_text:
		...
		if node.original_node.node_type == NodeType.ELEMENT_NODE:
			...
			if (
				...
				# Build the base line (prefix + tag name)
				if should_show_scroll and not node.is_interactive:
					...
					line = (
						f'{depth_str}{shadow_prefix}{new_prefix}{scroll_prefix}'
						f'{node.original_node.backend_node_id}]<{node.original_node.tag_name}'
					)
				...
				# NEW: annotate the tag based on backend_node_id metadata
				metadata_suffix = DOMTreeSerializer._build_node_metadata_suffix(
					node.original_node,
					node_metadata,
				)
				if metadata_suffix:
					line += metadata_suffix
			...
			for child in node.children:
				child_text = DOMTreeSerializer.serialize_tree(child, include_attributes, next_depth, node_metadata)
				if child_text:
					formatted_text.append(child_text)
	...
	@staticmethod
	def _build_node_metadata_suffix(
		node: EnhancedDOMTreeNode,
		node_metadata: dict[int, dict[str, Any]] | None,
	) -> str:
		"""
		Build extra inline markers based on per-node metadata.

		Example for clickable:
		    <a [CLICKABLE] ... />
		"""
		...
		meta = node_metadata.get(backend_id)
		if not meta:
			return ''

		# Example handling for a boolean 'clickable' flag
		parts: list[str] = []

		# 1) Special-case well-known flags
		clickable = meta.get('clickable')
		if isinstance(clickable, bool) and clickable:
			parts.append('[CLICKABLE]')

		# 2) Generic handling for other metadata entries
		#    Skip keys we already handled explicitly above.
		for key, value in meta.items():
			if key == 'clickable':
				continue
			# Allow future expansion, e.g. [priority=high], [group=foo]
			parts.append(f'[{key}={value}]')

		if not parts:
			return ''

		# Leading space so it can be appended directly after the tag name
		# e.g. "<a [CLICKABLE] id=... />"
		return ' ' + ' '.join(parts)

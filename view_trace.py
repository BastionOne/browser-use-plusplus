...
class HTTPServerHandler(http.server.SimpleHTTPRequestHandler):
    """Custom HTTP handler that serves the HTML file at root path."""
    ...
    def do_GET(self):
        ...
        if self.path == "/" or self.path == "/index.html":
            ...
            self.end_headers()
            self.wfile.write(b"<h1>404 - Not Found</h1><p>Only the root path (/) is available.</p>")
...
def serve_html_content(html_content: str, port: int = 8099) -> None:
    """
    Serve HTML content via HTTP server on specified port.
    Runs until interrupted with Ctrl+C.
    """
    # Create handler with HTML content
    handler = lambda *args, **kwargs: HTTPServerHandler(*args, html_content=html_content, **kwargs)
    
    try:
        with socketserver.TCPServer(("", port), handler) as httpd:
            print(f"🌐 Serving agent viewer at: http://localhost:{port}")
            print("📖 Open the URL above in your browser to view the agent execution trace")
        ...
        if e.errno == 98:  # Address already in use
            ...
            print(f"💡 Try stopping any existing server on port {port} or use a different port")
        else:
...
def main():
    ...
    # Serve via HTTP server on Linux
    print("Starting HTTP server...")
    serve_html_content(html_content, port=8099)
